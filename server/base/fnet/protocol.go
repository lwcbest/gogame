...
func (this *Protocol) OnConnectionMade(fconn iface.Iconnection) {
	logger.Info(fmt.Sprintf("client ID: %d connected. IP Address: %s", fconn.GetSessionId(), fconn.RemoteIp()))
...
func (this *Protocol) OnConnectionLost(fconn iface.Iconnection) {
	logger.Info("client ID: %d disconnected. IP Address: %s", fconn.GetSessionId(), fconn.RemoteIp())
...
func (this *Protocol) StartReadThread(fconn iface.Iconnection) {
	...
	for {
		...
		if err != nil {
			logger.Error("DoFrequencyControl err %v", err)
			fconn.Stop()
			return
		}
		pkg, err := utils.ReadPackage(fconn)
		if err != nil {
			eStr := err.Error()
			if strings.Contains(eStr, "close 1006") {
				logger.Info("[Warn][Disconnect]client disconnect...%v", err)
			} else {
				logger.Error("parse package failed! err: %+v", err)
			}
			fconn.Stop()
			return
		}
		if utils.GlobalObject.PoolSize > 0 {
			this.msghandle.DeliverToMsgQueue(pkg)
		} else {
			this.msghandle.DoMsgFromGoRoutine(pkg)
		}
	}
}

/* datapack 的方法不用了使用新的pkg和message两层解包这里先注释掉
func (this *Protocol) StartReadThread(fconn iface.Iconnection) {
	logger.Info("start receive data from socket...")
	for {
		//频率控制
		...
		logger.Info(fmt.Sprintf("msg id :%d, data len: %d", pkg.MsgId, pkg.Len))
		if utils.GlobalObject.PoolSize > 0 {
